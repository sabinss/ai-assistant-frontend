...
const tableHeader = [
    ...
    { name: "Frequency", sortable: false, sortKey: "frequency" },
...
const MemoizedTableRow = React.memo(({ item, index }: any) => {
    ...
    return (
        ...
        >
            <TableCell className="py-3 max-w-20 break-words">{truncateText(item.conversation.question, 20)}</TableCell>
            <TableCell className="py-3 max-w-20 break-words">{truncateText(item.feedback)}</TableCell>
            <TableCell className="py-3 max-w-20 break-words">{truncateText(item.conversation.answer)}</TableCell>
            <TableCell className="py-3 max-w-20 break-words">{truncateText(item.modified_answer)}</TableCell>
            <TableCell className="py-3">{item.frequency}</TableCell>
            <TableCell className="py-3">
                <div className={`w-fit rounded-sm px-2 text-white ${style}`}>{item.status}</div>
            </TableCell>
            <TableCell className="py-3">{dFormat(item?.createdAt as Date, 'yyyy-MM-dd h:m:s')}</TableCell>
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    This action cannot be undone. This will permanently delete your feedback and remove its data from our servers.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel>
                                ...
                                <AlertDialogAction className="px-5 py-1 bg-red-500 rounded-sm hover:bg-red-600">
                                    <button className="text-red-50 px-3 py-1" onClick={() => handleDelete(item._id)}>
                                        Delete
                                    </button>
                                </AlertDialogAction>
                            </AlertDialogFooter>
...
export default function FeedbackTable() {
    ...
    const { sortColumn, sortOrder, search, limit, page, feedbacks, endDate, startDate, status, feedbackType, removeState } = feedbackTable;
    ...
    useEffect(() => {
        ...
        fetchFeedbacks();
    }, [page, search, sortColumn, feedbackType, startDate, endDate, sortOrder, limit, removeState, status, feedbackType]);

    const fetchFeedbacks = async () => {
        try {
            setIsLoading(true);
            const response = await http.get<{ feedbacks: Feedback[]; totalPages: number }>(
    ...
    return (
        <div className="w-full bg-white rounded-md p-4 text-[#333333] relative">
            ...
            <div className="w-full flex flex-col gap-2 md:flex-row md:justify-end">
                <div>
                    <TableFilter parentComponent="feedbacktable" FilterComponent={FeedbackFilter} />
                </div>
                <div className="">
