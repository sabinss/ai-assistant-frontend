...
const ConvoGraph = () => {
    ...
    useEffect(() => {
        const fetchConversations = async () => {
            try {
                const response = await http.get(`/conversations/whole_organization?startDate=${fromDate}&endDate=${toDate}`, {
                    headers: {
        ...
        const processData = (conversationData) => {
            ...
            const startDate = new Date(Math.min(...conversations.map((conversation) => new Date(conversation.createdAt))));
            const endDate = new Date(Math.max(...conversations.map((conversation) => new Date(conversation.createdAt))));
            const dateRange = getDatesInRange(startDate, endDate, range);
            const chartData = {
                labels: dateRange.map((date) => formatISO(date, { representation: 'date' })),
                datasets: [
                    {
                        label: 'Total',
                        data: dateRange.map((date) => countConversationsOnDate(conversations, date)),
                        borderColor: 'blue',
                        backgroundColor: 'blue',
                    },
                    {
                        label: 'Liked',
                        data: dateRange.map((date) => countLikedConversationsOnDate(conversations, date)),
                        borderColor: 'green',
                        backgroundColor: 'green',
                    },
                    {
                        label: 'Disliked',
                        data: dateRange.map((date) => countDislikedConversationsOnDate(conversations, date)),
                        borderColor: 'red',
                        backgroundColor: 'red',
                    },
                    // {
    ...
    return (
        <main>
            <div className='w-full flex justify-between'>
                ...
                <div className="">
                    <CalendarDateRangePicker setFromDate={setFromDate} setToDate={setToDate} />
                </div>
            </div>
