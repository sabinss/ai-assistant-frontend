...
export default function EditProfile({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setRoles(
          roleData?.data.map((role: any) => ({ id: role._id, name: role.name }))
        )
        const { first_name, last_name, email, role, status } =
          userData?.data?.user
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await http.put(`/user/${user_id}`, updatedData, {
        headers: {
          Authorization: `Bearer ${access_token}`,
        },
      })
  ...
  return (
    <div className="w-full rounded-md bg-white p-4 text-[#333333]">
      ...
      <div className="userDetails flex flex-col gap-4 text-base">
        ...
        {/* Role */}
        <div className="flex flex-col items-center md:flex-row">
          <p className="mb-2 w-full font-semibold md:mb-0 md:w-1/3">Role:</p>
          <select
