...
const [formData,setFormData]=useState('');
  ...
  return (
    <div className="px-4">
      ...
      {error && (
        <div className="flex flex-col mt-2 text-red-700 font-normal bg-red-300 px-8 py-1 rounded-lg">{error}</div>
      )}
      {!enteredDetails ? (
        <form className="mt-4 flex flex-col gap-2" onSubmit={handleSubmit(onSubmit)}>
          <div className={`flex rounded-lg border-2 ${errors?.email ? 'border-red-700' : 'border-[#CCC]'} bg-[#E7E7E7] px-3 py-2`}>
            <input
          ...
          {errors?.email && (errors?.email?.type === "required" || errors?.email?.type === "pattern") && (
            <span role="alert" className="text-red-700 mt-[-10px] text-[12px]">
              {errors?.email?.type === 'pattern' ? 'Entered value does not match email format' : 'Email field is required'}
            </span>
          )}
          <div className={`flex rounded-lg border-2 ${errors?.first_name ? 'border-red-700' : 'border-[#CCC]'} bg-[#E7E7E7] px-3 py-2`}>
            <input
              className="w-full bg-transparent text-sm text-slate-600 outline-none"
              type="text"
          ...
          {errors?.first_name && errors?.first_name?.type === "required" && (
            <span role="alert" className="text-red-700 mt-[-10px] text-[12px]">First Name field is required</span>
          )}
          <div className={`flex rounded-lg border-2 ${errors?.last_name ? 'border-red-700' : 'border-[#CCC]'} bg-[#E7E7E7] px-3 py-2`}>
            <input
              className="w-full bg-transparent text-sm text-slate-600 outline-none"
              type="text"
          ...
          {errors?.last_name && errors?.last_name?.type === "required" && (
            <span role="alert" className="text-red-700 mt-[-10px] text-[12px]">Last Name field is required</span>
          )}
          <div className={`flex rounded-lg border-2 ${errors?.organization_name ? 'border-red-700' : 'border-[#CCC]'} bg-[#E7E7E7] px-3 py-2`}>
            <input
              className="w-full bg-transparent text-sm text-slate-600 outline-none"
              type="text"
            ...
            />
            <div className="border-l-2 border-[#CCC] pl-2">
              <Landmark className={`${errors?.organization_name ? "text-red-700" : "text-[#CCC]"}`} />
            </div>
          </div>
          {errors?.organization_name && errors?.organization_name?.type === "required" && (
            <span role="alert" className="text-red-700 mt-[-10px] text-[12px]">Organization Name field is required</span>
          )}
          <div className={`flex rounded-lg border-2 ${errors?.ai_assistant_name ? 'border-red-700' : 'border-[#CCC]'} bg-[#E7E7E7] px-3 py-2`}>
            <input
              className="w-full bg-transparent text-sm text-slate-600 outline-none"
              type="text"
          ...
          {errors?.ai_assistant_name && errors?.ai_assistant_name?.type === "required" && (
            <span role="alert" className="text-red-700 mt-[-10px] text-[12px]">AI Assistant field is required</span>
          )}
          <div className={`flex rounded-lg border-2 ${errors?.password ? 'border-red-700' : 'border-[#CCC]'} bg-[#E7E7E7] px-3 py-2`}>
            <input
              className="w-full bg-transparent text-sm text-slate-600 outline-none"
              type={togglePass}
            ...
            <div className="border-l-2 border-[#CCC] pl-2">
              {togglePass === 'password' ? (
                <EyeIcon onClick={togglePassword} className={errors?.password ? "text-red-700" : "text-[#a2a2a2]"} />
              ) : (
                <EyeOffIcon onClick={togglePassword} className="text-[#a3a3a3] cursor-pointer" />
              )}
            </div>
          </div>
          {errors?.password && errors?.password?.type === "required" && (
            <span role="alert" className="text-red-700 mt-[-10px] text-[12px]">Password field is required</span>
          )}
          {isSubmitting ? (
...
const EmailConfirmDialog = ({ formData }) => {
  ...
  return (
    <div className="mt-4 flex flex-col items-center">
      <h1 className="text-center text-2xl mt-2 font-semibold">Email Confirmation</h1>
      <input
