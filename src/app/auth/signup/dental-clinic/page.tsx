...
export default function Page() {
    ...
    const DENTAL_CLINIC_INDUSTRY = "Dental Clinic"
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (data) => {
        const payload = { ...data, industry: DENTAL_CLINIC_INDUSTRY }
        const success = await sendConfirmEmail(payload)
        if (success) {
            setFormData(payload)
...
const sendConfirmEmail = async (formData: any): Promise<boolean> => {
    try {
        await http.post("/auth/sendEmailVerifyToken", {
            ...
            industry: formData.industry ?? "Dental Clinic",
        })
