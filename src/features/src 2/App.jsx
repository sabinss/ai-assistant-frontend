...
export default function App() {
  ...
  async function handleSaveDone({ outcome, notes }) {
    ...
    setActions(prev =>
      prev.map(a => a.id === modalAction.id ? { ...a, done: true, doneOutcome: outcomeLabel } : a)
    );
  ...
  if (loading) {
    return (
      <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F4F6FA', fontFamily: 'Inter, sans-serif', color: '#8B91A3', fontSize: 13 }}>
        Loading…
      </div>
    );
  }
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', background: '#F4F6FA', color: '#1A1F2E', height: '100vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* Page header */}
          <div style={{ background: '#fff', borderBottom: '1px solid #E2E6EF', padding: '11px 24px', display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
            <div>
            ...
            <div style={{ marginLeft: 'auto' }}>
              <span style={{ background: '#E8EDF8', color: '#1B3A8C', fontSize: 12, fontWeight: 500, padding: '5px 14px', borderRadius: 99 }}>
                {todayCount} account{todayCount !== 1 ? 's' : ''} need{todayCount === 1 ? 's' : ''} attention today
              </span>
            </div>
          </div>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '18px 22px' }}>
              ...
              {/* Promoted banner */}
              <div style={{
                background: '#E8EDF8', border: '1px solid #C0CCE8', borderRadius: 8,
                padding: '8px 14px', marginBottom: 16, fontSize: 12.5, color: '#1B3A8C',
