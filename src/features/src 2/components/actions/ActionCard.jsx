...
function ScoreChip({ label, level }) {
...
function Btn({ onClick, color, hoverBg, borderColor, children }) {
  ...
  return (
    <button
      ...
      style={{
        fontFamily: 'inherit', fontSize: 12, fontWeight: 500, padding: '6px 13px',
...
export default function ActionCard({ action, onGetDraft, onMarkDone, onViewAccount, onSnooze, onToast }) {
  const [hovered, setHovered] = useState(false);
  ...
  // Build score chips
  const chips = [];
  if (action.scores.risk != null) chips.push({ label: `Risk ${action.scores.risk}`, level: action.scores.riskLevel });
  if (action.scores.value != null) chips.push({ label: `Value ${action.scores.value}`, level: action.scores.valueLevel });
  if (action.scores.opp != null) chips.push({ label: `Opp ${action.scores.opp}`, level: action.scores.oppLevel });
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 10, padding: 15, marginBottom: 8,
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#1A1F2E', marginBottom: 3, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            {action.company}
            <Badge variant={`stage-${action.stage}`}>{action.stageLabel}</Badge>
            {action.renewal && (
              <Badge variant={action.renewalUrgent ? 'renewal-urgent' : 'renewal'}>Renewal {action.renewal}</Badge>
            )}
      ...
      {chips.length > 0 && (
        <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
          {chips.map((c, i) => <ScoreChip key={i} label={c.label} level={c.level} />)}
        </div>
      )}

      {/* Why now */}
      <div
        style={{ fontSize: 12.5, color: '#4A5168', lineHeight: 1.6, marginBottom: 12, padding: '10px 12px', background: '#F4F6FA', borderRadius: 8, border: '1px solid #E2E6EF' }}
        ...
        <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
          {action.draftKey && (
            <Btn onClick={() => onGetDraft(action)} color="#1B3A8C" hoverBg="#E8EDF8">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4z"/></svg>
              {action.draftLabel || 'Get Draft'}
            </Btn>
          )}
          {action.tier !== 'watch' && (
            <Btn onClick={() => onMarkDone(action)} color="#2E7D52" hoverBg="#F0F9F4" borderColor="#2E7D52">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
              Mark Done
            </Btn>
          )}
          ...
          {action.tier === 'today' && (
            <button
              ...
              style={{ background: 'transparent', color: '#8B91A3', border: '1px solid transparent', fontSize: 11.5, padding: '6px 13px', borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit', marginLeft: 'auto' }}
            >
              Snooze
            </button>
          )}
