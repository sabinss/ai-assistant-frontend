...
export default function ChatPanel({ pendingDraft, onDraftHandled, onToast }) {
  const [messages, setMessages] = useState([
    {
      id: 1, role: 'agent', type: 'text', time: '9:02 AM',
      content: "Good morning. You have <strong>2 TODAY</strong> actions. Northgate's champion departure is the most time-sensitive — the successor window is open now. Want me to draft the outreach?",
    },
  ]);
  ...
  function addUserMessage(content) {
    setMessages(prev => [...prev, { id: nextId(), role: 'user', type: 'text', content, time: getTime() }]);
  }

  function addAgentMessage(content) {
    setMessages(prev => [...prev, { id: nextId(), role: 'agent', type: 'text', content, time: getTime() }]);
  }

  function addDraftMessage(subject, body) {
    setMessages(prev => [...prev, { id: nextId(), role: 'agent', type: 'draft', subject, body, time: getTime() }]);
  }
  ...
  return (
    <div style={{
      ...
      borderLeft: '1px solid #E2E6EF', display: 'flex', flexDirection: 'column', background: '#fff',
    }}>
      {/* Header */}
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #E2E6EF', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          width: 30, height: 30, borderRadius: '50%', background: '#1B3A8C',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0,
        }}>🤖</div>
        <div>
          <div style={{ fontSize: 13.5, fontWeight: 600 }}>Chat with Astro</div>
        ...
        <div style={{ marginLeft: 'auto' }}>
          <button
            ...
            style={{
              fontSize: 12, color: '#1B3A8C', fontWeight: 500, cursor: 'pointer',
              display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', fontFamily: 'inherit',
            }}
          >
      ...
      {/* Messages */}
      <div style={{
        flex: 1, overflowY: 'auto', padding: '14px', display: 'flex', flexDirection: 'column', gap: 10, background: '#F4F6FA',
      }}>
        {messages.map(msg => (
        ...
        {isTyping && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            <div style={{
              ...
              {[0, 150, 300].map(d => (
                <span key={d} style={{
                  width: 5, height: 5, background: '#8B91A3', borderRadius: '50%', display: 'inline-block',
      ...
      {/* Agent chips */}
      <div style={{ padding: '10px 12px', borderTop: '1px solid #E2E6EF', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {AGENTS.map(agent => (
          <button
            key={agent.id}
            onClick={() => { setActiveAgent(agent.id); onToast(`Switched to ${agent.label}`); }}
            style={{
      ...
      <div style={{ padding: '10px 12px 12px', borderTop: '1px solid #E2E6EF' }}>
        <div style={{
          ...
          border: '1px solid #CDD3E0', borderRadius: 8, padding: '8px 10px',
          background: '#fff',
        }}>
          <textarea
            value={input}
            ...
            style={{
              ...
              fontSize: 13, color: '#1A1F2E', background: 'transparent',
              resize: 'none', maxHeight: 80, minHeight: 20,
            }}
            rows={1}
          />
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <button
              onClick={handleSend}
              style={{
                width: 28, height: 28, borderRadius: '50%', background: '#1B3A8C',
