...
export default function DoneModal({ action, onSave, onCancel }) {
  ...
  const overlayStyle = {
    ...
    opacity: action ? 1 : 0, pointerEvents: action ? 'all' : 'none', transition: 'opacity 0.18s',
  };
  const modalStyle = {
    background: '#fff', border: '1px solid #CDD3E0', borderRadius: 12,
    padding: 24, width: 400, boxShadow: '0 8px 32px rgba(0,0,0,0.1)',
    transform: action ? 'translateY(0)' : 'translateY(6px)', transition: 'transform 0.18s',
  };
    ...
    <div style={overlayStyle} onClick={e => e.target === e.currentTarget && handleCancel()}>
      <div style={modalStyle}>
        ...
        <select
          ...
          style={{
            width: '100%', background: '#fff', border: '1px solid #CDD3E0', borderRadius: 6,
            color: '#1A1F2E', fontFamily: 'inherit', fontSize: 13, padding: '8px 10px',
            marginBottom: 12, appearance: 'none', cursor: 'pointer',
          }}
        >
        ...
        <textarea
          ...
          style={{
            width: '100%', background: '#fff', border: '1px solid #CDD3E0', borderRadius: 6,
            color: '#1A1F2E', fontFamily: 'inherit', fontSize: 13, padding: '8px 10px',
            marginBottom: 16, resize: 'none', height: 80,
            outline: 'none',
          }}
        />
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={handleCancel} style={{
            background: '#fff', color: '#4A5168', border: '1px solid #CDD3E0',
            padding: '7px 16px', fontSize: 13, borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit',
          }}>Cancel</button>
          <button onClick={handleSave} style={{
            background: '#1B3A8C', color: '#fff', border: 'none',
            padding: '7px 20px', fontSize: 13, borderRadius: 6, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500,
          }}>Save &amp; Close</button>
        </div>
      </div>
    </div>
