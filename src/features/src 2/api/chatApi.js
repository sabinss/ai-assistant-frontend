// ─────────────────────────────────────────────
// CHAT API
// Handles chat message responses and draft
// generation. Swap mock responses for real
// AI/LLM endpoint calls when ready.
// ─────────────────────────────────────────────
...
/**
 * Send a chat message and receive a text response.
 * @param {string} text     — user message
 * @param {string} agentId  — active agent context
 * @returns {Promise<{ type: 'text', content: string }>}
 */
export async function sendChatMessage(text, agentId) {
  // TODO: replace with real LLM endpoint
...
/**
 * Request a draft for a specific action.
 * @param {string} account   — account identifier key
 * @param {string} draftType — draft type key
 * @returns {Promise<{ type: 'draft', subject: string, body: string } | { type: 'text', content: string }>}
 */
export async function requestDraft(account, draftType) {
  // TODO: replace with real LLM/template endpoint
  // return fetch('/api/drafts', {
  //   method: 'POST',
  //   headers: { 'Content-Type': 'application/json' },
  //   body: JSON.stringify({ account, draftType }),
  // }).then(r => r.json());
