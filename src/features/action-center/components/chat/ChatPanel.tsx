...
import type { ChatMessageItem, PendingDraft } from "../../types"
...
export default function ChatPanel({ pendingDraft, onDraftHandled, onToast }: ChatPanelProps) {
  ...
  const handleDraftRequest = useCallback(async (account: string, draftType: string) => {
    setIsTyping(true)
    ...
    if (result.type === "draft") {
      setMessages((prev) => [
        ...prev,
        { id: nextId(), role: "agent", type: "draft", subject: result.subject, body: result.body, time: getTime() },
      ])
    } else {
      setMessages((prev) => [
        ...prev,
        { id: nextId(), role: "agent", type: "text", content: result.content, time: getTime() },
      ])
    }
  }, [])
  ...
  useEffect(() => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: nextId(), role: "user", type: "text", content: `Get draft — ${label}`, time: getTime() },
    ])
    void handleDraftRequest(account, draftType)
    onDraftHandled()
  }, [pendingDraft, handleDraftRequest, onDraftHandled])
